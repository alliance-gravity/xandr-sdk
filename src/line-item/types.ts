...
interface ImpressionTracker {
  ...
  code: string;
  state: State;
  publisher: BasicIdName;
  tag: BasicIdName[];
  payment_rule: BasicIdName;
  line_item: BasicIdName;
  last_modified: string;
}
...
export interface LineItem {
  ...
  total_days: number;
  advertiser: BasicIdName;
  labels: ('Sales Rep' | 'Trafficker')[];
  pixels: Pixel[];
  insertion_orders: InsertionOrder[];
  goal_pixels: GoalPixel[];
  imptrackers: ImpressionTracker[];
  valuation: Valuation;
  creatives: Creative[];
  budget_intervals: BudgetInterval[];
  enable_pacing: boolean;
  ...
  supply_strategies: SupplyStrategy;
